...
func TestGRPCServerConfigFromEnvironment(t *testing.T) {
	...
	t.Setenv("DURABLETASK_GRPC_ALLOWED_CLIENT_URIS", "urn:durabletask:client-a, urn:durabletask:client-b")
	...
	require.Equal(t, []string{"urn:durabletask:client-a", "urn:durabletask:client-b"}, config.allowedClientURIs)
}

func TestAuthorizeClientRequiresAllowedURI(t *testing.T) {
	allowedURI, err := url.Parse("urn:durabletask:client")
	require.NoError(t, err)
	...
	otherURI, err := url.Parse("urn:durabletask:other")
	require.NoError(t, err)
